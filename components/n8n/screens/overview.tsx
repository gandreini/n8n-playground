...
function StatCard({ label, value, trend, trendDown }: StatCardProps) {
  return (
    <div className="flex-1 p-4 bg-[var(--color--neutral-white)] border border-[var(--color--neutral-150)] rounded-[var(--radius--xs)]">
      <p className="text-[var(--font-size--xs)] text-[var(--color--neutral-500)] mb-1">{label}</p>
      <div className="flex items-baseline gap-2">
        <span className="text-[var(--font-size--2xl)] font-[var(--font-weight--bold)] text-[var(--color--neutral-800)]">
          {value}
        </span>
        {trend && (
          <span className={cn(
            'text-[var(--font-size--2xs)]',
            trendDown ? 'text-[var(--color--red-500)]' : 'text-[var(--color--neutral-400)]'
          )}>
            {trendDown ? '▽' : '⌃'}{trend}
          </span>
        )}
    ...
    >
      ...
      <div className="flex items-center gap-3">
        <div className="flex items-center gap-2">
          ...
          {workflow.tags?.includes('Production Workflows') && (
            <span className="px-2 py-1 text-[var(--font-size--2xs)] border border-[var(--color--neutral-200)] rounded-[var(--radius--3xs)] flex items-center gap-1 bg-[var(--color--green-50)]">
              <Square className="w-3 h-3 fill-[var(--color--green-500)] text-[var(--color--green-500)]" />
              Produ...
            </span>
          )}
        </div>
...
function Pagination({ total, currentPage = 1 }: { total: number; currentPage?: number }) {
  return (
    ...
    <div className="h-full flex flex-col overflow-hidden bg-[var(--color--neutral-white)]">
      ...
      <div className="px-6 border-b border-[var(--color--neutral-150)]">
        <div className="flex">
          ...
          <TabButton tab="variables" label="Variables" active={currentTab === 'variables'} onClick={() => setTab('variables')} />
      ...
      <div className="flex-1 overflow-hidden flex flex-col">
        ...
        {/* Pagination */}
        {(currentTab === 'workflows' || currentTab === 'credentials' || currentTab === 'executions') && (
          <Pagination total={currentTab === 'workflows' ? 308 : currentTab === 'credentials' ? credentials.length : executions.length} />
        )}
      </div>
    </div>
  )
